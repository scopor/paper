...
md.use(MarkdownItContainer, 'bilibili', {
    ...
    render: function (tokens: any, idx: number): string {
        ...
        if (tokens[idx].nesting === 1) {
            return `<div id="bilibili"><iframe style="width: 100%; aspect-ratio: 16 / 9;" src="//player.bilibili.com/player.html?isOutside=1&bvid=` + md.utils.escapeHtml(m[1]) + `&p=1&autoplay=1&high_quality=1&as_wide=1&muted=0"></iframe></div>`;
        } else {
            return "";
        }
    }
